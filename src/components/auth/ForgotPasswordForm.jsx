...
export const ForgotPasswordForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSubmitted(true);
      toast.success(res.data?.message || "Đã gửi hướng dẫn qua email!");
    } catch (err) {
    ...
    return (
      <div className="flex flex-col items-center text-center gap-4 py-2">
        ...
        <div className="flex flex-col gap-1">
          <h3 className="font-semibold text-white text-base">Kiểm tra hộp thư của bạn</h3>
          <p className="text-xs text-[#908fa0] leading-relaxed max-w-[280px]">
            Nếu email <span className="text-white font-medium">{email}</span> tồn tại trong hệ thống, chúng tôi đã gửi đường dẫn đặt lại mật khẩu.
          </p>
        </div>
        <Link
          to="/login"
          className="mt-2 text-xs font-mono text-[#c0c1ff] hover:text-white transition-colors flex items-center gap-1"
        >
  ...
  return (
    <form className="flex flex-col gap-4 w-full" onSubmit={handleSubmit}>
      <div className="flex flex-col gap-1 text-center mb-1">
        <h2 className="text-lg font-bold text-white tracking-wide">Quên mật khẩu?</h2>
